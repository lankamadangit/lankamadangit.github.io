...
if($category){
	...
	if(!eregi("[^[:space:]]+",$key)) {
		...
		//코멘트도 함께 검색(2005-05-14)
		if($keyfield=="short"){
			$query="select count(*) from {$top}_board_{$code} a, {$top}_short_{$code} b where a.uid=b.parent and (a.comment LIKE '%$key%' or b.comment LIKE '%$key%')";
		} else {
...
for( $i=0; $i < $notice_num; $i++ ) {
	...
	if($my_filename){   
		if(eregi("\.jpg",$my_filename)||eregi("\.gif",$my_filename)||eregi("\.png",$my_filename)) {
			...
			//이미지 미리보기
			$my_filename_ecd=urldecode($my_filename);
			$thum_image="<img src=\"winko/data/$code/$my_filename\" width=150 border=0>";
...
while($row = mysql_fetch_array($result,MYSQL_ASSOC)) {
	...
	if($my_filename){   
		if(eregi("\.jpg",$my_filename)||eregi("\.gif",$my_filename)||eregi("\.png",$my_filename)) {
			...
			//이미지 미리보기
			$my_filename_ecd=urldecode($my_filename);
			$thum_image="<img src=\"winko/data/$code/$my_filename\" width=150 border=0 style='border:solid 1px #CCCCCC;'>";
		} elseif(eregi("\.zip",$my_filename)) {
			$real_file = "{$save_dir}/{$my_filename}";
			$file_icon = "&nbsp;<A href='winko/include/etc_down_hit.php?code=$code&v=$v&category=$category&page=$page&number=$my_uid&keyfield=$keyfield&key=$encoded_key&file=$my_filename'><img src='winko/system/winko_img/zip.gif' width=17 height=17 border=0></a>";
		} elseif(eregi("\.pdf",$my_filename)) {
		...
		$file_icon = "";
		$thum_image="<img src='{$skin_folder}/no_img.gif' width='150' border='0'>";
	}
...
include $skin_folder."/view_list_blank.php"; 

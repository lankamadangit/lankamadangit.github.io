...
<?=$hide_end?>
   ...
   <tr height=28>
      ...
      <td><?=$hide_html_start?>&nbsp;<input type=checkbox name=ok_html value=1>(HTML)<?=$hide_html_end?><?=$hide_notice_start?>&nbsp;&nbsp;<INPUT type=checkbox value=1 name="notice">(Notice)<?=$hide_notice_end?><?=$hide_secret_start?><?if($member[no]){?>&nbsp;&nbsp;<INPUT type=checkbox value=1 name="ok_secret">(비밀글)<?}?><?=$hide_secret_end?></td>
   </tr>

   <tr height=28>
      <td width="18%" align="right" bgcolor=<?=$light?>><img src="<?=$skin_folder?>/main.gif" align='absmiddle'>&nbsp;Subject&nbsp;&nbsp;</td>
      <td>&nbsp;&nbsp;<input type="text" name="subject" size="70" maxlength="70" class='input'></td>
   </tr>

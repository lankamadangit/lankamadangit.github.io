...
$comment = addslashes($comment);
   ...
   $subject = "$admin_company Inquiry-Japanese ($name)";
   if($admin_email)
   {
    $mime_type="text/html";
    $mail_body=($body);
...
mysql_query("insert into {$top}_inquiry_jp (name, company, phone, fax, email, address, country, comment, reg_date, ip) values ('$name', '$company', '$phone', '$fax', '$email', '$address', '$country', '$comment', '$reg_date', '$ip')") or error("Error<br>".mysql_error());
...
?>
<meta http-equiv='refresh' content='0; url=../../../jp_main.html'>
</body>
</table>